class TicTacToe {
    ...
    victory() {
        ...
        this.victoryText.innerHTML = `<span>${this.player === 'X' ? 'Player One Wins!' : 'Player Two Wins!'}</span><br><span class="overlay-text-small text-alert">Play Again!</span>`
    ...
    showDrawMessage() {
        this.gameActive = false // End the game
        this.victoryText.classList.add('visible')
        this.victoryText.innerHTML = `It's a Tie!<br><span class="overlay-text-small text-alert">Play Again!</span>`
        this.victoryText.addEventListener('click', () => this.initializeGame())
        const restartButton = document.getElementById('btn')
        restartButton.style.display = 'block' // Show the restart button
        restartButton.addEventListener('click', () => this.initializeGame())
